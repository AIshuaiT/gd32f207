...
int lwip_system_init(void)
{
    ...
    if(!is_dhcp_enable)
    {
        if (netif_default != RT_NULL)
        {
            ...
            ipaddr.addr = g_wnc_config.net_config.ip;
            netmask.addr = g_wnc_config.net_config.netmask;
            gw.addr = g_wnc_config.net_config.gateway;

            netifapi_netif_set_addr(netif_default, (const ip4_addr_t *)&ipaddr, (const ip4_addr_t *)&netmask, (const ip4_addr_t *)&gw);
        }
    }
