/**
  ...
  * @version V1.0.0
  * @date    15-Jul-2015
  * @brief   ETH functions of the firmware library.
  ******************************************************************************
  */

/**
 * Apr 26th, 2017. By Test.
 * Add code for RT-Thread, see line 2844.
 */
 #include <rtthread.h>
...
//    /* KSZ8863 not support softreset via this interface */
...
//    _eth_delay_(PHY_RESETDELAY);
    ...
    if(ETH_InitParaStruct->ETH_MAC_AutoNegotiation != ETH_AUTONEGOTIATION_DISABLE)
    {
        ...
        /* KSZ8863 has different registers, set to 100m full-duplex directly */
 ...
 ******************************************************************************
 * DATE            BY           DESCRIPTION
 * 2017-04-26      Test          First version. Add eth interface to RTT.
 ******************************************************************************
 */
...
/**
 * @brief  RTT eth device object structure
 */
struct rt_gd32_eth
{
...
ALIGN(4)
static rt_uint8_t Rx_Buff[ETH_RXBUFNB][ETH_MAX_FRAME_SIZE];
ALIGN(4)
static rt_uint8_t Tx_Buff[ETH_TXBUFNB][ETH_MAX_FRAME_SIZE];
...
static rt_size_t    rt_gd32_eth_read    (rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size);
static rt_size_t    rt_gd32_eth_write   (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size);
static rt_err_t     rt_gd32_eth_control (rt_device_t dev, rt_uint8_t cmd, void *args);
/* ethernet device interface */
/* transmit packet. */
rt_err_t            rt_gd32_eth_tx      (rt_device_t dev, struct pbuf* p);
/* reception packet. */
struct pbuf *       rt_gd32_eth_rx      (rt_device_t dev);
/* RT-Thread Device Interface End */
...
void ETH_IRQHandler(void)
{
    ...
    if ( (status & ETH_DMA_INT_R) != (u32)RESET ) /* packet receiption */
    {
    ...
    if ( (status & ETH_DMA_INT_T) != (u32)RESET ) /* packet transmission */
    {
        rt_sem_release(&tx_buf_free);
        ETH_DMAClearIntBitState(ETH_DMA_INT_T);
    }

    /* Clear received IT */
    if ((status & ETH_DMA_INT_NIS) != (u32)RESET)
        ETH_DMA->STR = (u32)ETH_DMA_INT_NIS;
    if ((status & ETH_DMA_INT_AIS) != (u32)RESET)
        ETH_DMA->STR = (u32)ETH_DMA_INT_AIS;
    if ((status & ETH_DMA_INT_RO) != (u32)RESET)
        ETH_DMA->STR = (u32)ETH_DMA_INT_RO;

    if ((status & ETH_DMA_INT_RBU) != (u32)RESET)
    {
        ETH_ResumeDMAReception();
        ETH_DMA->STR = (u32)ETH_DMA_INT_RBU;
    }
...
static rt_err_t rt_gd32_eth_init(rt_device_t dev)
{
    ...
    ETH_InitStructure.ETH_MAC_ChecksumOffload = ETH_CHECKSUMOFFLOAD_ENABLE;
    ...
    ETH_DMAINTConfig(ETH_DMA_INT_NIS | ETH_DMA_INT_R | ETH_DMA_INT_T, ENABLE);
    ...
    ETH_SetMACAddress(ETH_MAC_ADDRESS0, (u8*)&gd32_eth_device.dev_addr[0]);
...
static rt_err_t rt_gd32_eth_control(rt_device_t dev, rt_uint8_t cmd, void *args)
{
    ...
    case NIOCTL_GADDR:
        /* get mac address */
        if(args) rt_memcpy(args, gd32_eth_device.dev_addr, 6);
...
rt_err_t rt_gd32_eth_tx( rt_device_t dev, struct pbuf* p)
{
    ...
    for (q = p; q != NULL; q = q->next)
    {
        ...
        while (len)
        {
            (*(__IO uint8_t *)((DMACurrentTxDesc->Buffer1Addr) + offset)) = *ptr;
    ...
    DMACurrentTxDesc->ControlBufferSize = (p->tot_len & ETH_DMATXDESC_TB1S);
    /* Setting the last segment and first segment bits (in this case a frame is transmitted in one descriptor) */
    DMACurrentTxDesc->Status |= ETH_DMATXDESC_LSG | ETH_DMATXDESC_FSG;
    /* Enable TX Completion Interrupt */
    DMACurrentTxDesc->Status |= ETH_DMATXDESC_INTC;

#if CHECKSUM_BY_HARDWARE
    DMACurrentTxDesc->Status |= ETH_DMATXDESC_CM_TCPUDPICMP_FULL;
#endif /* CHECKSUM_BY_HARDWARE */
...
struct pbuf *rt_gd32_eth_rx(rt_device_t dev)
{
            ...
            ((DMACurrentRxDesc->Status & ETH_DMARXDESC_FDES) != (uint32_t)RESET))
    {
        ...
        framelength = ((DMACurrentRxDesc->Status & ETH_DMARXDESC_FRML) >> ETH_DMARXDESC_FRAME_LENGTHSHIFT) - 4;
        ...
        if (p != RT_NULL)
        {
            ...
            from = (const char *)(DMACurrentRxDesc->Buffer1Addr);
    ...
    else /* Ring Mode */
    {
        ...
        {
            /* Selects the next DMA Rx descriptor list for next buffer to read */
            DMACurrentRxDesc = (ETH_DMADESCTypeDef*) ((uint32_t)DMACurrentRxDesc + 0x10 + ((ETH_DMA->BCR & ETH_DMA_BCR_DPSL) >> 2));
        }
    }
...
/**
  * @brief  Configure the reset and clock control unit.
  * @param  None
  * @retval None
  */
static void RCC_Configuration(void)
...
/**
 * @brief  initailize eth hardware on gd32f20x
 */
void rt_hw_gd32_eth_init(void)
{
    RCC_Configuration();
    GPIO_Configuration();
    NVIC_Configuration();
...
/**
 * @brief  register eth device to RTT device interface
 * @param  mac[6] - MAC address register to device
 */
void rt_register_eth_dev(uint8_t mac[6])
{
    rt_memset(&gd32_eth_device, 0, sizeof(gd32_eth_device));
    
    /* MAC address */
    gd32_eth_device.dev_addr[0] = mac[0];
    gd32_eth_device.dev_addr[1] = mac[1];
    gd32_eth_device.dev_addr[2] = mac[2];
    gd32_eth_device.dev_addr[3] = mac[3];
    gd32_eth_device.dev_addr[4] = mac[4];
    gd32_eth_device.dev_addr[5] = mac[5];
    ...
    /* register eth device */
    eth_device_init(&(gd32_eth_device.parent), "e0");
}

/**
 * @NOTE  VERY IMPORTANT!!
 *        the three function below not very safe because operate variables 
 *        in very low level, this might be not very good way to do so. 
 */

/**
 * @brief  get device ip address
 * @retval ip address in hex format
 */
rt_uint32_t get_ip_addr(void)
{
    struct eth_device * eth_dev;
    rt_uint32_t         ret;
    
    eth_dev = (struct eth_device *)(&(gd32_eth_device.parent));
    
    if(eth_dev->netif != RT_NULL)
    {
        ret = eth_dev->netif->ip_addr.addr;
    }
    else
    {
        ret = 0;
...
/**
 * @brief  get device gateway address
 * @retval gateway address in hex format
 */
rt_uint32_t get_gw_addr(void)
{
    struct eth_device *eth_dev;
    rt_uint32_t    ret;
 ...
 * @retval netmask in hex format
 */
rt_uint32_t get_netmask(void)
{
    struct eth_device *eth_dev;
    rt_uint32_t    ret;
    ...
    if(eth_dev->netif != RT_NULL)
    {
        ret = eth_dev->netif->netmask.addr;
    }
    else
    {
        ret = 0;
...
/**
 * @brief  restart low level network
 * @retval RT_EOK for restart success, others means failed
 */
rt_err_t network_restart(void)
{
    struct eth_device * eth_dev;    
    rt_err_t            ret;
    ...
    if(eth_dev != RT_NULL)
    {
        ...
        netif_set_down(eth_dev->netif); 
        
        /* get device object */
        device = (rt_device_t) eth_dev;
        ...
        if (!(eth_dev->flags & ETHIF_LINK_PHYUP))
        {
