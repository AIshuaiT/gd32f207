...
/** @addtogroup GD32f20x_System_Private_Includes
  * @{
  */
...
/** @addtogroup GD32f20x_System_Private_Variables
  * @{
  */
...
#define  SYS_GCFGR_RESET                                 ((uint32_t)0xE0FF0000)
...
#define  SYS_GCFGR_PLLSEL_PLLPREDV_PLLMF_USBPS_RESET     ((uint32_t)0xDF00FFFF)
...
#define  SYS_GCIR_INT_FLAG_RESET                         ((uint32_t)0x00FF0000)
...
/* RCC GCFGR3 reset */
#define  SYS_GCFGR3_RESET                                   ((uint32_t)0x00000000)

/* RCC GPLLTIR_INT ans FLAG masks */
#define  SYS_GPLLTIR_INT_FLAG_RESET                         ((uint32_t)0x00400000)

/* RCC PLLTCFGR reset */
#define  SYS_PLLTCFGR_RESET                                 ((uint32_t)0x20003010)
...
#elif defined SYSCLK_FREQ_120MHz_HSI
  static void SetSysClockTo120HSI(void);
#endif
...
void SystemInit (void)
{
    ...
    /* Reset GCFGR3 register */
    RCC->GCFGR3 =SYS_GCFGR3_RESET ;

    /* Disable all interrupts and clear flag bits */
    RCC->GPLLTIR = SYS_GPLLTIR_INT_FLAG_RESET;

    /* Reset PLLTCFGR register */
    RCC->PLLTCFGR = SYS_PLLTCFGR_RESET;
...
#elif defined SYSCLK_FREQ_120MHz
/**
  * @brief  Set System clock frequency to 120MHz and configure AHB, APB1, and APB2 prescalers. 
  * @note   This function should be used only after reset.
  * @param  None
  * @retval None
  */
static void SetSysClockTo120(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* PLL configuration: PLLCLK = PREDIV1 * 10 = 120 MHz */ 
    RCC->GCFGR &= (uint32_t)~(RCC_GCFGR_PLLPREDV | RCC_GCFGR_PLLSEL | RCC_GCFGR_PLLMF);
    RCC->GCFGR |= (uint32_t)(RCC_GCFGR_PLLPREDV_PREDIV1 | RCC_GCFGR_PLLSEL_PREDIV1 | RCC_GCFGR_PLLMF10); 
...
static void SetSysClockTo120HSI(void)
{
  ...
  /* CK_SYS, AHB, APB2 and APB1 configuration ---------------------------*/    
    ...
    RCC->GCFGR &= (uint32_t)((uint32_t)~(RCC_GCFGR_PLLSEL | RCC_GCFGR_PLLPREDV | RCC_GCFGR_PLLMF));
    RCC->GCFGR |= (uint32_t)( RCC_GCFGR_PLLSEL_HSI_DIV2 | RCC_GCFGR_PLLMF30);
